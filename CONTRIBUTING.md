...
- Vendor column profiles ship `"verified": false` because no Australian payroll vendor publishes its export header list. Flipping a profile to `"verified": true` requires a real header row from that vendor's export — headers only, no data — quoted in the pull request.
