...
CLI that reads a pay-run/contribution CSV and, for each super contribution line, computes the statutory payday-super deadline, gives an on-time/late verdict, and produces an experimental SG-charge exposure estimate for late lines. Audience: Australian accountants and employers. Not a payroll system, not legal advice.

## Legal basis (current 2026-08-15)

The current pass read the Federal Register, the ATO legal database, direct ATO rate guidance and all eight official jurisdiction holiday sources. F2026L00784, SGAR regulations 11–13D, the $270,830 maximum contribution base and LCR 2026/1–3 are verified directly. LCR 2026/D1 remains draft pending appeal, so qualifying-earnings classification remains outside the runtime. Full source links, runtime comparisons and residuals are in `primary-source-review-2026-08-15.md`.
...
- Out-of-cycle QE (s 18C(2) item 2; F2026L00784): deadline = end of the usual period of the first later standard QE day. The earlier review's “LI 2026/20” shorthand is not displayed on the Federal Register as-made page, so the project pins the controlling registered identifier and text. The instrument requires that subsequent non-out-of-cycle payment, so a flagged row without it is rejected. There is no fallback.
...
- Transition: LCR 2026/1 applies pre-1 July contributions only to the extent they are unused old-regime excess, and applies 1–28 July contributions first against the employee's June-quarter shortfall. The CSV cannot hold those balances, so the operator must reconcile and pass `--confirm-transition-allocation`; the confirmation is recorded per affected row.
...
- `calendar.py` - loads bundled `paydaysuper/data/business_days.json` (+ optional user override JSON), `is_business_day(date)`, `add_business_days(date, n)`, horizon warning when computation leaves the range the table covers. `coverage_until` is that range's end: the bundled `verified_until`, raised only by an override's own `verified_until`, so a user who enters the missing years and says how far they went gets real verdicts. Holding a holiday is not evidence of completeness and never raises it. Past the coverage end the table may be missing holidays and a deadline computed across it can only be too early, which decides one side and not the other: a date on or before that deadline is provably on time, and only a date after it is left `UNKNOWN`. Provisional entries are advisory only and are treated as business days until an official-source override confirms them, avoiding a false on-time verdict.
...
- `importers.py` - the `payday-super-check import` half: reads a payroll export and a super payments export through those profiles, matches each payment to the payday it settles, and writes the canonical contributions CSV via `atomic_io`. No vendor export carries a fund receipt date, so every vendor date lands in `remitted` and `fund_received_date` is left blank. `_amount` quantises an amount as it reads it, through `report.cents`, so the canonical file it writes and the figure the checker reads back are one rounding of the input rather than two.
- `csv_io.py` - column mapping (CLI flags or JSON config), date parsing (ISO + DD/MM/YYYY), validation with loud errors. Payroll exports vary too much to guess at: a value the parser cannot read is named, never coerced. `_parse_amount` does not round: a checker input keeps the scale it arrived with.
...
- `atomic_io.py` - generated-output boundary for both halves: an explicit `.csv` destination, staged in that destination's own directory and moved into place with `os.replace`, so an existing symlink at the destination is replaced rather than followed and a failed write leaves no partial file at the chosen name.
- `cli.py` - argparse entry (`payday-super-check pay.csv --map field=column ...`), `--as-at` for the interest end date (default: today), `--assessment-date` for the s 18D cut-off and the explicit LCR 2026/1 transition confirmation. Exits non-zero (`report.needs_attention`) when LATE or UNPAID lines exist **or** when any line was left horizon-indeterminate `UNKNOWN`, which the verdict list below explains (scheduling-friendly).
