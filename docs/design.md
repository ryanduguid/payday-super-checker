...
- `cli.py` - argparse entry (`payday-super-check pay.csv --map field=column ...`), `--as-at` for the interest end date (default: today), `--assessment-date` for the s 18D cut-off, the explicit LCR 2026/1 transition confirmation and the importer's separate LCR 2026/2 allocation confirmation. Exits non-zero (`report.needs_attention`) for LATE/UNPAID or any attention-driving indeterminate row.
...
- `UNKNOWN` - quiet where a supported deadline has not passed or the row carries no SG. Attention-driving where missing holiday facts or unevidenced item 4 facts change the outcome. This includes post-horizon later dates, unfunded rows and stale pre-payments, plus a later row between its evidenced deadline and a possible item 4 upper bound. These cases name both candidates in the console/CSV, carry no exposure and drive exit code 2.
...
- Item 4 overlap: an evidenced on-time earlier contribution extends payday 2; missing/remittance-only/late evidence does not, and material uncertainty is non-zero `UNKNOWN`.
