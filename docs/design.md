...
The rules below were researched against primary sources and then re-checked by an independent verification pass. Most were read from the Act, the regulations or PCG 2026/1 directly. The GIC rate table was verified directly against the ATO page on 14 August 2026. Some other inputs still rest on secondary commentary because ato.gov.au and the ATO legal database can block automated fetching: the maximum contributions base, the text of LI 2026/20, and the finalisation status of LCR 2026/D1 to D4. Those need a browser check before release. Full citations, confidence ratings and open ambiguities in `docs/research-notes-2026-08-02.md`.
...
- `report.py` - console summary + `report.csv` (verdict, due date, pathway, days late and the date it was measured to, shortfall, NEC, uplift range low/high, caveats and notes per line), plus totals across every exposed line. This is the checker's rounding boundary: `money`/`cents` quantise to cents with `ROUND_HALF_UP` on the way out, and `_rounded_figures` rounds each component once and builds the totals from the rounded parts so a row's columns add up. **That rounding rule is an implementation choice, not a verified one.** No statutory or ATO rule for rounding an individual SG amount (cents or nearest dollar, per employee or per QE day) was ever cited or verified - `research-notes-2026-08-02.md` still carries it as unresolved - so the LCR 2026/D3 worked examples have to be checked against this arithmetic before release.
